  ...
  methods: {
    async submitForm() {
      ...
      if (this.onSelectNewForm) {
        ...
        let response = await MatchGoalService.update(
          matchGoalObject,
          this.matchId
        );
    ...
    async getAllItems() {
      ...
      for (var i = 0; i < items._embedded.matchGoalModelList.length; i++) {
        let type = this.splitType(
          items._embedded.matchGoalModelList[i].goalType.typeName,
          "_"
        );
        options[i] = {
          ...
          Type: type,
          Match:
            items._embedded.matchGoalModelList[i].match.homeTeam.association
              .name +
            " vs. " +
            items._embedded.matchGoalModelList[i].match.awayTeam.association
              .name +
            " # " +
            items._embedded.matchGoalModelList[i].match.matchDate,
          Description: items._embedded.matchGoalModelList[
            i
